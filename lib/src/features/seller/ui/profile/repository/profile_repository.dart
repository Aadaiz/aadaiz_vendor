...
class ProfileRepository {
  ...
  Future<MyProfileRes> addressApi({required dynamic body , dynamic module}) async {
    try {
      var response = await _http.post(module=='seller'? Api.selleraddress:Api.tailorAddress, body, contentType: true);
  ...
  Future<MyPaymentRes> getpaymentlistApi()async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var token = prefs.getString('token');
    var response = await _http.get('${Api.payment}?token=$token');
    final res = jsonDecode(response);
    MyPaymentRes data = MyPaymentRes.fromMap(res);
    return data;
  }
  ...
  Future<dynamic> updateprofileApi({required dynamic body}) async {
    try {
      var response = await _http.post(Api.updateProfile, body, contentType: true);
      log("Raw API Response: $response");
