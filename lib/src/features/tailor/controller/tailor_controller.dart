...
import '../../auth/kyc_status_screen.dart';
import '../model/category_list_model.dart';
import '../model/category_list_model.dart' as category;
import '../ui/dashboard/tailor_dashboard.dart';
...
class TailorController extends GetxController {
  ...
  Future<dynamic> tailorKyc(context) async {
    if (acc.text != conacc.text) {
      CommonToast.show(
        msg: "Confirm Account number should be same as Account number",
      );
    } else if (ifsc.text == '') {
...
var fcmToken = prefs.getString('fcmToken');
      ...
      Map body = {
        ...
        'category': selectedCategoriesWithPrice.map(
          (key, value) => MapEntry(key.toString(), value.toString()),
        ),
        'mobile_number': AuthController.to.mobile.text,
        'what_u_done':
            '${AuthController.to.upload[0]},${AuthController.to.upload[1]},${AuthController.to.upload[2]}',
      ...
      if (res.success == true) {
        ...
        if (AuthController.to.kycStatus == 0 || AuthController.to.kycStatus == 2) {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (context) => KYCPendingScreen(type: 'tailor'),
            ),
          );
        } else {
          Navigator.of(context).pushReplacement(
  ...
  Future<dynamic> getAddressList() async {
    ...
    Map body = {'action': 'list', 'token': '$token'};
